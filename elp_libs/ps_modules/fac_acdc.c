...
#define WFMREF_DECIMATION       (uint16_t) (CONTROL_FREQ / WFMREF_FREQ)

#define TIMESLICER_BUFFER       1
#define BUFFER_FREQ             g_ipc_mtoc.control.freq_timeslicer[TIMESLICER_BUFFER]
#define BUFFER_DECIMATION       (uint16_t) (CONTROL_FREQ / BUFFER_FREQ)
...
#define V_CAPBANK_CONTROLLER_DECIMATION     (uint16_t) (CONTROL_FREQ / V_CAPBANK_CONTROLLER_FREQ)
...
static float samplesBuffer[BUFFER_SIZE];
...
static void init_controller(void)
{
    ...
    init_dsp_error(ERROR_IOUT_RECT, &g_controller_ctom.net_signals[5],
                   &IOUT_RECT, &g_controller_ctom.net_signals[6]);
    ...
    init_buffer(BUF_SAMPLES, &samplesBuffer, BUFFER_SIZE);
...
static void turn_on(uint16_t dummy)
{
    ...
    #endif
    {
        ...
        else
        {
            ...
            enable_pwm_output(PWM_MODULATOR);
        }
    }
}
...
static void turn_off(uint16_t dummy)
{
    disable_pwm_output(PWM_MODULATOR);
