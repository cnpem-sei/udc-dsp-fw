...
void Init_DMA_32bits_McBSP(void);
void Init_DMA_McBSP_nBuffers(Uint16 n_buffers, Uint16 size_buffers);
void Init_DMA_McBSP_nBuffers2(Uint16 n_buffers, Uint16 size_buffers);
void start_DMA(void);
void stop_DMA(void);
...
//*****************************************************************************
// DMA Initialization for a McBSP data size > 16-bit and <= 32-bit
//*****************************************************************************
void Init_DMA_32bits_McBSP(void)
{
  i_rdata = 0;
  ...
  //
  //DmaRegs.CH1.TRANSFER_SIZE = (TRANSFER_BUFFER_SIZE-2)/2;
  //DmaRegs.CH2.TRANSFER_SIZE = (TRANSFER_BUFFER_SIZE-2)/2;
  DmaRegs.CH2.TRANSFER_SIZE = DMA_TRANSFER_SIZE-1;
  DmaRegs.CH1.TRANSFER_SIZE = DMA_TRANSFER_SIZE-1;
  //
  ...
  //DmaRegs.CH1.DST_WRAP_SIZE = DECIMATION_FACTOR-1;
  ...
  DmaRegs.CH2.MODE.bit.CHINTE = 0; // Disable
...
//*****************************************************************************
// DMA Initialization for a McBSP data size > 16-bit and <= 32-bit
// filling n buffers. These buffers must be allocated contiguously
// in a RAM memory.
//*****************************************************************************
void Init_DMA_McBSP_nBuffers2(Uint16 n_buffers, Uint16 size_buffers)
{
  ...
  DmaRegs.CH3.BURST_SIZE.all = 1;
  DmaRegs.CH3.SRC_BURST_STEP = 1;
  ...
  DmaRegs.CH2.SRC_TRANSFER_STEP = 0xFFFF;
  DmaRegs.CH2.DST_TRANSFER_STEP = 0xFFFF;
  DmaRegs.CH3.SRC_TRANSFER_STEP = 0xFFFF;
  DmaRegs.CH3.DST_TRANSFER_STEP = 0xFFFF;
  //
  // For receive, after each burst:
  //     Subtract 1 from the source (back to DRR2)
  //     Add 3 to the destination: move to the next word
  ...
  DmaRegs.CH2.SRC_ADDR_SHADOW = (Uint32) &dummy_data;
  DmaRegs.CH2.DST_ADDR_SHADOW = (Uint32) &McbspaRegs.DXR2.all;
  DmaRegs.CH3.SRC_ADDR_SHADOW = (Uint32) &dummy_data;
  DmaRegs.CH3.DST_ADDR_SHADOW = (Uint32) &McbspaRegs.DXR2.all;
  //
  // Receive source and destination:
  //   McBSP (source) -> DMA -> receive buffer (destination)
  //   Set the source to the McBSP DRR2
  ...
  DmaRegs.CH1.MODE.bit.CONTINUOUS = 1;
  DmaRegs.CH2.MODE.bit.CONTINUOUS = 1;
  DmaRegs.CH3.MODE.bit.CONTINUOUS = 0;
  ...
  DmaRegs.CH3.MODE.bit.CHINTE = 0; // Disable
